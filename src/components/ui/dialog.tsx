...
>(({ className, children, ...props }, ref) => (
  <DialogPortal>
    ...
    <DialogPrimitive.Content
      ref={ref}
      className={cn(
        // `overflow-x-hidden` porque quase todo diálogo daqui pede
        // `overflow-y-auto`, e só isso já libera a rolagem lateral junto (o CSS
        // promove o outro eixo a `auto`). O que for largo de verdade lá dentro
        // — uma imagem grande, um bloco de log — traz o próprio scroller.
        "fixed left-[50%] top-[50%] z-50 grid w-full max-w-lg translate-x-[-50%] translate-y-[-50%] gap-5 overflow-x-hidden rounded-2xl border border-border bg-card p-6 shadow-xl duration-200 data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:fade-out-0 data-[state=open]:fade-in-0 data-[state=closed]:zoom-out-95 data-[state=open]:zoom-in-95",
        className,
      )}
      {...props}
    >
